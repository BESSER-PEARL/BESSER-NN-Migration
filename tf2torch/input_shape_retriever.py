...
from tf2torch.definitions import layers_buml2tf
...
def extract_nn_code(file_path: str, nn_type: str):
    ...
    seq_code = []
    in_init = True
    imports = "from tf2torch.input_shape_retriever import get_input_shape_all\n"
    imports += "from tf2torch.input_shape_retriever import get_data\n"
    if nn_type == "sequential":
        seq_code, nn_name = extract_sequential_blocks(file_path)
        seq_code = [imports] + seq_code
    else:
    ...
    return nn_name, init_code, call_code, seq_code


def get_data(input_shape: tuple | None):
    """
...
def extract_and_modify_code(file_path: str, shape: tuple | None,
                            input_nn_type: str):
    """
    Extracts code defining the NN model.
    ...
    nn_name, init_code, call_code, seq_code = (
...
def execute_code(code: str):
    ...
    try:
        result = subprocess.run([sys.executable, file_path], capture_output=True,
...
def extract_sequential_blocks(filepath: str):
    """
    Extracts lines of source code corresponding to nn.Sequential
    model definitions from the Python file, and identifies the name
    of main NN model.

    Parameters:
        filepath (str): Path to the Python source file to parse.

    Returns:
        tuple:
            - list[str]: A list of source code lines, starting with
                all import statements found in the file, followed by
                lines belonging to all detected nn.Sequential blocks.
            - str | None: The name of the main model, identified as
                the Sequential that references other Sequential
                variables. Falls back to the last defined Sequential
                if no such reference is found, or None if no
                Sequential blocks are detected.
    """
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Assign) and isinstance(node.value, ast.Call):
            ...
            if (isinstance(func, ast.Attribute) and func.attr == "Sequential") or \
               (isinstance(func, ast.Name) and func.id == "Sequential"):
                name = node.targets[0].id
                sequential_names.append(name)
                blocks[name] = lines[node.lineno - 1 : node.end_lineno]

    # main model: the Sequential that references other Sequential variables
    main_model = None
    for name in sequential_names:
        node_lines = "".join(blocks[name])
        referenced = [n for n in sequential_names if n != name and n in node_lines]
        if referenced:
            main_model = name
            break

    # fallback: last defined
    if main_model is None and sequential_names:
        main_model = sequential_names[-1]

    all_lines = import_lines + [line for block in blocks.values() for line in block]
    return all_lines, main_model
